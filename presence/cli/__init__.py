...
from presence import __app_name__, __banner__
...
@click.option('--log-level', default='INFO', metavar='LEVEL', callback=handle_log_level, help='Either CRITICAL, ERROR, WARNING, INFO or DEBUG')
def cli(log_level):
    ...
    try:
        ...
        if exc.errno == errno.EEXIST and os.path.isdir(config_dir):
            ...
            log.critical('Cannot create application directory "{}"'.format(config_dir))
            sys.exit(1)
    ...
    if ctx.invoked_subcommand is None:
        prompt_kwargs = {
            'history': FileHistory(os.path.join(click.get_app_dir(__app_name__), 'history')),
            'message': 'Presence> ',
        }
